...
CMD  printenv > /etc/environment && ln -snf /usr/share/zoneinfo/$TZ /etc/localtime && echo $TZ > /etc/timezone  && sed -i 's@CRON_PLACEHOLDER@'"$CRON"'@g' /etc/cron.d/crontab && service cron restart && tail -f /var/log/cron.log
