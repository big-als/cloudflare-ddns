...
$date = Get-Date -Format "dd/MM/yyyy HH:mm UTCK"
...
if (-not($auth_result.result)) {
    Write-Output "[$($date)] API token validation failed. Error: $($auth_result.errors.message). Terminating script."
    # Exit script
    return
}
...
if (-not($DnsZone.result)) {
    Write-Output "[$($date)] Search for the DNS domain [$($Domain)] return zero results. Terminating script."
    # Exit script
    return
}
## Store the DNS zone ID
$zone_id = $DnsZone.result.id
Write-Output "[$($date)] Domain zone [$($Domain)]: ID=$($zone_id)"
#End Region
...
if (-not($DnsRecord.result)) {
    Write-Output "[$($date)] Search for the DNS record [$($Record)] return zero results. Terminating script."
    # Exit script
    return
}
...
Write-Output "[$($date)] DNS record [$($Record)]: Type=$($record_type), IP=$($old_ip)"
#EndRegion

#Region Get Current Public IP Address
$new_ip = Invoke-RestMethod -Uri 'https://v4.ident.me'
Write-Output "[$($date)] Public IP Address: OLD=$($old_ip), NEW=$($new_ip)"
#EndRegion
...
if ($new_ip -ne $old_ip) {
    Write-Output "[$($date)] The current IP address does not match the DNS record IP address. Attempt to update."
    ...
    if (($Update.errors)) {
        Write-Output "[$($date)] DNS record update failed. Error: $($Update[0].errors.message)"
        ## Exit script
        return
    }
    ...
    Write-Output "[$($date)] The current IP address and DNS record IP address are the same. There's no need to update."
}
#EndRegion
